...
//                     dump($command);//debug
    ...
    //检测用户是否存在或是否被禁用
    public function authExists($map, $model = '') {
        if (empty($model)) {
            $model = $this->_config['auth_user'];
        }
    ...
    public function saveAccessList($authId = null) {
        if (null === $authId) {
            $authId = session(config('auth_config.user_auth_key'));
        }
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if (config('auth_config.auth_type') != 2 && !session(config('auth_config.user_auth_key'))) {
            session('ACCESS_LIST', $this->getAuthList($authId, 1));
        }
        return;
    }
    ...
    public function getUserAccess($uid) {
        ...
        if (empty($accessList)) {
            $accessList = $this->getAuthList($uid, 1);
            if (!empty($accessList)) {
